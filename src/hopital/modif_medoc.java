...
public class modif_medoc implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					modif_medoc window = new modif_medoc();
	...
	public modif_medoc() {
		...
		JLabel lblNewLabel = new JLabel("Modifier le stock d'un médicament :");
		lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 14));
		lblNewLabel.setBounds(25, 11, 203, 23);
		layeredPane.add(lblNewLabel);
		
		JLabel lblNewLabel_1 = new JLabel("ID du médicament");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==modif) {
			...
			String requete = "UPDATE `USER`.`medicament` SET `MedStock` = '"+qt+"' WHERE (`idMedicament` = '"+s+"');"; 
			try {
				st.execute(requete);
				JOptionPane.showMessageDialog(frame, "Médicament modifié");
